...
class ControllerMarketingCoupon extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addCoupon:".$this->request->post['name'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editCoupon:ID=".$this->request->get['coupon_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $coupon_id) {
				$result['name'][] = $this->model_marketing_coupon->getCoupon($coupon_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['coupon_id'].',name='.$value['name'].';';
			}
			$done="deleteCoupon:".substr($name, 0, -1);
