...
// $data['checkout_title'] = $this->config->get('checkout_title');
    ...
    public function get_add_product() {
            ...
            $data['addproducts'] = array();
            $addproducts = $this->cart->getaddProducts();
            foreach ($addproducts as $addproduct) {
                 $productprice=$this->model_catalog_category->getAddProductPrices($addproduct['product_id'],$cart_total);  
                ...
                $json['addproducts'][] = array(
                    'product_id'   => $addproduct['product_id'], 
                    'name'   => $addproduct['name'], 
                    'quantity'   => $addproduct['quantity'], 
                    'image'   => $image,

                    'price'       => $this->currency->format($productprice['addprice'],$this->session->data['currency']),
                    'product_id'   => $addproduct['product_id'], 
                    'option_data'   => $addproduct['option_data']
                    );
            }
...
// 获取参与加购的产品
    ...
    public function add_product_to_addcart() {
        ...
        if ($product_info) {
            ...
            $product_options = $this->model_catalog_product->getAddcartProductOptions($this->request->post['product_id'],$product_option_value_id);
            ...
            if (!$json) {
                ...
                $this->cart->addproduct($this->request->post['product_id'], $quantity, $option, $recurring_id);
