...
class ControllerCheckoutCart extends Controller {
    public function index() {
        ...
        if(isset($_SERVER['HTTP_REFERER'])){
            ...
            $data['addproduct'] =$this->url->link('checkout/cart/add_ydproduct');
 // $data['error_warning']='';
            ...
            foreach ($products as $product) {
               ...
               $wishlist= $this->model_catalog_product->wishlistornot($product['product_id']);
                $data['products'][] = array(
                    ...
                    'qty'       => $product['qty']>=1000?'999+':$product['qty'],
                    'qtys'      => $product['qty'],
...
//            print_r( $products);exit();
        ...
        $this->response->setOutput($this->load->view('checkout/add_ydproduct', $data));
             // print_r($data['addproducts']);exit;
    }
    ...
    public function add_ydproduct() {
        ...
        $products = $this->cart->getProducts($cart_ids);
           $cart_total=0;
            ...
            if(isset($_SERVER['HTTP_REFERER'])){
                $data['gooback'] =$_SERVER['HTTP_REFERER'];
            }else{
                $data['gooback'] =$this->url->link('checkout/cart');
            }
            $data['gotoback'] =$this->url->link('checkout/cart');
             $this->response->setOutput($this->load->view('checkout/cart_add_ydproduct', $data));
