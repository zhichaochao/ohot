					<!-- 移动端定制主页面内容部分 -->
					<?php if(!$ydresultdata){?>
					<!-- <div class="ydh_care_a">
						<p>Add Customized Wig</p>
						<span><img src="catalog/view/theme/default/img/png/yd_img.png"></span>
					</div> -->
					<div class="ydh_care_b ydh_mized">
						<span class="ydh_span">01</span>
						<button class="ydh_button ydh_cers" data="1" >ADD HAIR BUNDLES</button>
						<button class="ydh_button" data="6">ADD CLOSURE / FRONTAL</button>
					</div>
						
					<?php }else{?>
						<div class="ydh_care_d ydh_mized">
						<span class="ydh_span">02</span>
						<ul class="ydh_ul">

						<?php foreach($ydresultdata as $productdata){ ?>
		
							<li  class="<?=$productdata['status']==1 ?'lian':'kuai';?>">
								<i  onclick="delcustom('<?php echo $productdata['custom_product_id']; ?>',this);" ><img src="catalog/view/theme/default/img/png/yd_img1.png"></i>
						...
						<button class="ydh_button ydh_cers" data="1">CONTINUE TO ADD HAIR BUNDLES</button>
						<button class="ydh_button ydh_button_a" data="6">ADD CLOSURE / FRONTAL</button>
						...
						<?php if ($option_info) { ?>
							...
							<div class="hwm_free hwm_ty">
								...
								<ul>
									...
									<!-- <input type="hidden" name="customoption[<?=$option['option_id']?>]"  value="<?php echo $option['option_value_data'][0]['option_value_id'];?>"> -->
								</ul>
							</div>
					...
					$(function(){
						//替换图片
							...
							if(win<=992){
								...
								// 弹窗
								$(".ydh_button").click(function(){
									var catalog=$(this).attr('data');
										 $.ajax({
								            url: 'index.php?route=information/selecatalog/ydselecatalog&catalog_id='+catalog,
						        			dataType: 'html',	
						        			success: function(html) {
								            		$('#yd-hwm-sion-confirm').html(html);
								            		$(".ydh_sure").fadeIn();
								            },
