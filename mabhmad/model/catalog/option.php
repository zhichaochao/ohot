...
class ModelCatalogOption extends Model {
	...
	//通过产品ID查找option_id
	public function getOptionIdByProductId($product_id){
		$query = $this->db->query("SELECT option_id FROM " . DB_PREFIX . "product_option WHERE product_id = '" . $product_id . "'");
	...
	//获取name为Color的option_id
	public function getColorOptionId(){
		$query = $this->db->query("SELECT option_id FROM " . DB_PREFIX . "option_description WHERE name = 'Color'");
		return $query->row['option_id'];
	}
	
	//获取name为Length的option_id
	public function getLengthOptionId(){
		$query = $this->db->query("SELECT option_id FROM " . DB_PREFIX . "option_description WHERE name = 'Length'");
		return $query->row['option_id'];
	}
}